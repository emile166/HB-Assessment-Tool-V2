...
function Differential2Questionnaire({ questionnaire, onBack, primaryResults }) {
    ...
    const handleSubmit = (finalResponses = responses) => {
        ...
        const lateralStressTestResponse = responses?.[DIFFERENTIAL_2_DATA.lateralStressTest.id];
        const hasJointLaxity = Array.isArray(lateralStressTestResponse)
            ? lateralStressTestResponse.some(answer => answer.id === "lateralStressTestAnswer4")
            : lateralStressTestResponse?.id === "lateralStressTestAnswer4";

        // Calculate results based on the provided logic
        let resultsSummary;
        if (hasJointLaxity) {
            resultsSummary = "🚨 Seek evaluation from a medical professional.";
        ...
        if (/💪|⚡/.test(resultsSummary)) {
            ...
            displayedResult = 'Joint laxity indicated';
        } else if (/🎊/.test(resultsSummary)) {
            displayedResult = `${firstInjuryName} and ${secondInjuryName}`;
        } else if (/🤔/.test(resultsSummary)) {
        ...
        if (displayedResult.toLowerCase() === "nerve issue") {
            ...
            additionalDetails = "You indicated joint laxity on the lateral stress test, which is a symptom of a more severe injury that this tool is not designed to handle. You should seek guidance from a qualified medical professional (ideally one knowledgeable about rock climbing injuries). If you'd like, you can schdeule an online or in-person appointment with Dr. Jason Hooper, PT, DPT, OCS, SCS on our website at the following link: https://www.hoopersbeta.com/private-sessions";//www.hoopersbeta.com/book-appointment</a>;
        }
    ...
    const checkForEarlyCompletion = (currentId) => {
        ...
        const passiveJointExtensionIndex = questionnaire.questions.findIndex(q => q.id === 'passiveJointExtension');
        const lateralStressTestResponse = responses?.[DIFFERENTIAL_2_DATA.lateralStressTest.id];

        console.log('Early completion check:', {
            currentQuestionIndex,
            passiveJointExtensionIndex,
            lateralStressTestResponse,
            isAtRightQuestion: currentQuestionIndex === passiveJointExtensionIndex - 1
        });

        if (currentQuestionIndex === passiveJointExtensionIndex - 1) {
            if (Array.isArray(lateralStressTestResponse)) {
                const hasJointLaxity = lateralStressTestResponse.some(answer => answer.id === "lateralStressTestAnswer4");
                console.log('Multiple answers case:', { hasJointLaxity });
                return hasJointLaxity;
            } else if (lateralStressTestResponse) {
                const hasJointLaxity = lateralStressTestResponse.id === "lateralStressTestAnswer4";
                console.log('Single answer case:', { hasJointLaxity });
                return hasJointLaxity;
            }
        }
        return false;
        ...
        return (
            <Layout>
                <Card ref={questionnaireContainerRef}>
                    <AppHeader />
                    <ResultsCard
                        title="Differential Assessment 2 Results"
...
export default Differential2Questionnaire;